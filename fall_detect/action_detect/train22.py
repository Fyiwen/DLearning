...
# 训练transformer加openpose
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
#DEVICE='cpu'
input_size = 32  # 这里尺寸大小没办法啊，GPU花太多,所以本来骨骼图就是128的，要更小，就32吧
hidden_size = 64
num_layers = 2
output_size = 2
frames_per_clip = 2
...
model = TransformerNet(frames_per_clip, input_size, input_size,channels=1).to(DEVICE)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        data = data.to(DEVICE)  # 输入x的形状(batch_size,frames_per_clip, 1, 图像高度, 图像宽度)
    ...
    torch.save(model.state_dict(),
               f"F:/bishe/fall_detect/action_detect/checkPoint/train/Transformeraction.jit")  # 在这个路径下保存训练得到的模型的每一层参数
