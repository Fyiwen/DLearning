...
#DEVICE='cpu'
input_size = 16  # 这里尺寸大小没办法啊，GPU花太多
hidden_size = 16
num_layers = 2
output_size = 2
frames_per_clip = 1
...
model = TransformerNet(frames_per_clip, input_size, input_size,channels=3).to(DEVICE)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        data = data.to(DEVICE)  # 输入x的形状(batch_size,frames_per_clip, 3, 图像高度, 图像宽度)
