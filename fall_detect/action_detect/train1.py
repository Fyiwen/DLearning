...
model = LSTM(input_size * input_size * 3, hidden_size, num_layers, output_size).to(DEVICE)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (data, targets) in enumerate(train_loader):
        ...
        #label_tags = torch.argmax(targets, dim=1)
    ...
    test_score = sum_score.item() / len(test_loader)
